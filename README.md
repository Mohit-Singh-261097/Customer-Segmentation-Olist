...
![Segment Radar Chart](visuals/cluster_radar_chart.png)

### Cluster Distribution

![Cluster Distribution](visuals/cluster_distribution.png)
...
**Challenge:** The Olist dataset contains 9 relational tables, and a naive merge would create heavy duplication. A single order can contain multiple items, each with its own shipping-related fields — inflating the dataset and distorting customer behavior analysis.

**Solution:** Aggregation-first approach before merging: collapsed item-level data into order-level metrics, then aggregated to customer-level features using `customer_unique_id` as the final analytical grain.

### 2. Handling Extreme Data Skew
**Challenge:** Features such as Total Price and Freight Value showed strong right-skew, biasing K-Means centroids toward outliers.

**Solution:** Applied log transformation using `log(x + 1)` to normalize highly skewed monetary and weight-related variables, improving cluster separation between low, medium, and high spenders.

### 3. Reducing High-Dimensional Noise
**Challenge:** Initial feature engineering produced 16+ features — several were highly correlated or near-constant across most customers.

**Solution:** Feature selection by removing features with correlation > 0.90 and dropping near-static features where ~97% of customers shared the same value, keeping only features that added meaningful behavioral signal.
...
~29% of customers fall into the Dissatisfied At-Risk segment. Proactive delay notifications, service recovery messaging, and targeted recovery vouchers could retain 5–10% of customers who might otherwise churn, protecting future GMV and reducing revenue loss from poor delivery experiences.

### 2. Improved Marketing Efficiency for Budget Casuals
47% of the customer base, but low repeat purchase behavior. Personalized cross-sell campaigns based on previous purchases could improve CTR, increase second-purchase conversion, and reduce CAC for repeat buyers.

### 3. Logistics Optimization for Remote Regionals
~10% of customers in the Northeast experience 20-day average shipping. Regional fulfillment centers, localized inventory, and specialized courier partnerships could shift these customers into higher satisfaction segments.
...
[View Customer Segment Comparison Dashboard (PDF)](dashboard/Customer_segmentation_dashboard.pdf)
...
[GitHub](https://github.com/Mohit-Singh-261097) · [LinkedIn](https://www.linkedin.com/in/mohit-singh-661427181/)
